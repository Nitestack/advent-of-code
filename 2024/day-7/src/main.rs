    ...
    let result_part1 = input
        ...
        .map(|line| {
            ...
            let numbers = unparsed_numbers
                ...
                .collect::<Vec<_>>();
            if try_numbers(&numbers[1..], result, numbers[0]) {
                result
            } else {
                0
            }
    ...
    let result_part2 = input
        ...
        .map(|line| {
            ...
            let numbers = unparsed_numbers
                ...
                .map(|unparsed_number| unparsed_number.parse::<usize>().unwrap())
                .collect::<Vec<_>>();
            if try_numbers2(&numbers[1..], result, numbers[0]) {
                result
            } else {
                0
            }
...
fn try_numbers(numbers: &[usize], result: usize, current: usize) -> bool {
    if numbers.is_empty() {
        ...
        try_numbers(&numbers[1..], result, current + numbers[0])
            || try_numbers(&numbers[1..], result, current * numbers[0])
    }
}
...
fn try_numbers2(numbers: &[usize], result: usize, current: usize) -> bool {
    if numbers.is_empty() {
        ...
        try_numbers2(&numbers[1..], result, current + numbers[0])
            || try_numbers2(&numbers[1..], result, current * numbers[0])
            || try_numbers2(
                &numbers[1..],
                result,
                current * 10usize.pow(numbers[0].ilog10() + 1) + numbers[0],
            )
    }
}
